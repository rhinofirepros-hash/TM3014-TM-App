...
const OtherTable = ({ entries, onChange }) => {
  ...
  const updateEntry = (id, field, value) => {
    const updatedEntries = entries.map(entry => {
      if (entry.id === id) {
        ...
        if (field === 'quantityOfUnit' || field === 'quantityOfOther') {
          updated.total = (parseFloat(updated.quantityOfUnit) || 0) * (parseFloat(updated.quantityOfOther) || 0);
        }
        return updated;
      }
      return entry;
          ...
          {entries.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} className={`text-center py-8 ${themeClasses.text.secondary}`}>
                No other items added yet. Click "Add empty row" to get started.
              </TableCell>
            </TableRow>
          ) : (
            entries.map((entry) => (
              <TableRow key={entry.id}>
                <TableCell className="p-1">
                  <Input
                    ...
                    onChange={(e) => updateEntry(entry.id, 'otherName', e.target.value)}
                ...
                <TableCell className="p-1">
                  <Input
                    type="number"
                    value={entry.quantityOfOther}
                    onChange={(e) => updateEntry(entry.id, 'quantityOfOther', e.target.value)}
                ...
                <TableCell className="p-1">
                  <Input
                    type="number"
                    value={entry.quantityOfUnit}
                    onChange={(e) => updateEntry(entry.id, 'quantityOfUnit', e.target.value)}
