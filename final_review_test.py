...
Tests both server.py (legacy) and server_unified.py where applicable
...
class FinalReviewTester:
    def __init__(self):
        ...
        self.test_results = {
            ...
            "server_validation": {"passed": 0, "failed": 0, "errors": []},
    ...
    def test_health_check(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("health_check", "Health check endpoint", False, f"Status: {response.status_code}")
                return False
    ...
    def test_basic_connectivity(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("general", "Basic connectivity", False, f"Status: {response.status_code}")
                return False
        except Exception as e:
    ...
    def test_projects_gc_pin_fields(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("api_endpoints", "Projects GC PIN fields", False, f"Status: {response.status_code}")
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "gcPin" in pin_data and "projectId" in pin_data:
                    ...
                    self.log_result("gc_pin_system", "GC PIN generation", False, 
                                  "Missing gcPin or projectId in response")
            else:
                self.log_result("gc_pin_system", "GC PIN generation", False, f"Status: {response.status_code}")
        except Exception as e:
    ...
    def test_gc_pin_validation(self, pin):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result("gc_pin_system", "GC PIN validation", False, f"Status: {response.status_code}")
        except Exception as e:
            self.log_result("gc_pin_system", "GC PIN validation", False, str(e))
        return None
    
    def test_gc_pin_regeneration(self, pin_data):
        """Test that PIN regenerates after successful validation"""
        print(f"\n=== Testing GC PIN Regeneration ===")
        if not pin_data or "projectId" not in pin_data:
            self.log_result("gc_pin_system", "PIN regeneration setup", False, "No valid PIN data for regeneration test")
            return False
        
        project_id = pin_data["projectId"]
        original_pin = pin_data.get("gcPin")
        
        try:
            # Generate a new PIN for the same project
            response = self.session.get(f"{self.base_url}/projects/{project_id}/gc-pin")
            if response.status_code == 200:
                new_pin_data = response.json()
                new_pin = new_pin_data.get("gcPin")
                
                if new_pin and new_pin != original_pin:
                    self.log_result("gc_pin_system", "PIN regeneration", True, 
                                  f"PIN regenerated: {original_pin} -> {new_pin}")
                    return True
                else:
                    self.log_result("gc_pin_system", "PIN regeneration", False, 
                                  f"PIN not regenerated: {original_pin} -> {new_pin}")
            else:
                self.log_result("gc_pin_system", "PIN regeneration", False, f"Status: {response.status_code}")
        except Exception as e:
            self.log_result("gc_pin_system", "PIN regeneration", False, str(e))
        return False
    
    def test_crew_log_date_handling_with_server_switch(self):
        """Test crew log date handling, switching servers if needed"""
        print("\n=== Testing Crew Log Date Handling (2025-09-17) ===")
        
        # First try with current server
        success = self.attempt_crew_log_test()
        
        if not success:
            print("\n🔄 Crew logs not available on current server, attempting server switch...")
            
            # Try switching to legacy server
            if self.switch_to_legacy_server():
                print("✅ Switched to legacy server, retrying crew log test...")
                success = self.attempt_crew_log_test()
                
                # Switch back to unified server
                print("🔄 Switching back to unified server...")
                self.switch_back_to_unified_server()
            else:
                self.log_result("crew_log_date_handling", "Server switch", False, "Could not switch to legacy server")
        
        return success
    
    def attempt_crew_log_test(self):
        """Attempt to perform crew log test with current server"""
        # Get a project to work with
        try:
            response = self.session.get(f"{self.base_url}/projects")
            if response.status_code != 200:
                self.log_result("crew_log_date_handling", "Get projects for test", False, f"Status: {response.status_code}")
                return False
            
            projects = response.json()
            if not projects or len(projects) == 0:
                self.log_result("crew_log_date_handling", "Get projects for test", False, "No projects available")
                return False
            ...
            if response.status_code == 200:
                    ...
                    if matching_tm_tag:
                        ...
                        if expected_date in tm_date:
                            self.log_result("crew_log_date_handling", "T&M tag date handling", True, 
                                          f"T&M tag created with correct date: {tm_date}")
                ...
                self.log_result("crew_log_date_handling", "Crew log endpoint availability", False, 
                              "crew-logs endpoint not available on current server")
                return False
            else:
            ...
            self.log_result("crew_log_date_handling", "Crew log test", False, str(e))
            return False
    
    def switch_to_legacy_server(self):
        """Switch to legacy server.py for crew log testing"""
        try:
            # Update supervisor config to use server.py
            result = subprocess.run([
                'sudo', 'sed', '-i', 
                's/server_unified:app/server:app/g', 
                '/etc/supervisor/conf.d/supervisord.conf'
            ], capture_output=True, text=True)
            
            if result.returncode == 0:
                # Restart backend service
                restart_result = subprocess.run(['sudo', 'supervisorctl', 'restart', 'backend'], 
                                              capture_output=True, text=True)
                if restart_result.returncode == 0:
                    self.log_result("server_validation", "Switch to legacy server", True, 
                                  "Successfully switched to server.py")
                    time.sleep(5)  # Wait for server to start
                    return True
                else:
                    self.log_result("server_validation", "Switch to legacy server", False, 
                                  f"Failed to restart backend")
            else:
                self.log_result("server_validation", "Switch to legacy server", False, 
                              f"Failed to update config")
        except Exception as e:
            self.log_result("server_validation", "Switch to legacy server", False, str(e))
        return False
    
    def switch_back_to_unified_server(self):
        """Switch back to unified server"""
        try:
            # Update supervisor config to use server_unified.py
            result = subprocess.run([
                'sudo', 'sed', '-i', 
                's/server:app/server_unified:app/g', 
                '/etc/supervisor/conf.d/supervisord.conf'
            ], capture_output=True, text=True)
            ...
            if result.returncode == 0:
                ...
                restart_result = subprocess.run(['sudo', 'supervisorctl', 'restart', 'backend'], 
                                              capture_output=True, text=True)
                if restart_result.returncode == 0:
                    self.log_result("server_validation", "Switch back to unified server", True, 
                                  "Successfully switched back to server_unified.py")
                    time.sleep(5)  # Wait for server to start
                    return True
        except Exception as e:
            self.log_result("server_validation", "Switch back to unified server", False, str(e))
        return False
    ...
    def run_final_review_tests(self):
        ...
        print("🎯 STARTING FINAL REVIEW TESTING")
        print("=" * 60)
        ...
        if projects and len(projects) > 0:
            ...
            if pin_data and "gcPin" in pin_data:
                validation_result = self.test_gc_pin_validation(pin_data["gcPin"])
                
                # Test PIN regeneration if validation was successful
                if validation_result and not validation_result.get("used_pin"):
                    self.test_gc_pin_regeneration(pin_data)
        
        # Test 6: Crew log date handling (with server switching if needed)
        self.test_crew_log_date_handling_with_server_switch()
...
def main():
    ...
    print("🎯 FINAL REVIEW REQUEST BACKEND TESTING")
    ...
    print("6. Both server.py (legacy) and server_unified.py validation")
    print()
    ...
    if success:
        ...
        print("\n❌ Final review testing encountered critical issues!")
        sys.exit(1)
